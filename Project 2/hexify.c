...
void usage() {
   fprintf(stderr, "usage: hexify [-ajcpyrh] <input>\n");
   ...
   fprintf(stderr, "   -h output as hex string\n");
   exit(1);
}

typedef enum {ARRAY, JAVA, STRING, PERL, PYTHON, RUBY, HEX} type;
...
int doString(int fd, char *name, int output) {
   ...
   while (1) {
      ...
      printf(output == HEX ? "%02.2x" : "\\x%02.2x", ch);
      idx++;
      if (idx == lineLen && output != HEX) {
        printf("\"");
         idx = 0;
      }
   }
